  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">เว็บบอร์ดสนทนา</h1>
          <p className="text-gray-600">เลือกหมวดหมู่เพื่อดูหรือสร้างกระทู้ (ข้อมูลตัวอย่าง)</p>
        </div>
        {user?.role === "admin" && (
          <Button>
          ...
          <Link key={category.id} href={`/dashboard/discussion/${category.id}`}>
            <Card className="h-full hover:shadow-lg transition-shadow cursor-pointer">
              <CardContent className="p-6 flex flex-col justify-between h-full">
                ...
                <CardDescription className="text-gray-600 text-sm mb-4">{category.description}</CardDescription>
      ...
      {filteredCategories.length === 0 && (
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-500">ลองเปลี่ยนคำค้นหา</p>
          </CardContent>
        </Card>
      )}
    </div>
